...
class Store {
    ...
    static getProductById(productId) {
        try {
            return storage.get("products").find((p) => p.id == productId).value()
        } catch (err) {
            throw new BadRequestError(err);
        }
    ...
    static newPurchase({shoppingCart, user}) {
        ...
        if (!user.name || !user.email) {
            throw new BadRequestError("User info not complete, missing name or email")
        }
        ...
        try {
            shoppingCart.map((item) => {
                if (!item.itemId || !item.quantity) {
                    throw new BadRequestError("One of the items does not have item id or quantity field")
                }
                ...
                let currAmount = Store.getProductById(item.itemId).price * item.quantity;
                receipt.push(`${item.quantity} total ${Store.getProductById(item.itemId).name} purchased at a cost of $${Store.getProductById(item.itemId).price.toFixed(2)} for a total cost of $${currAmount.toFixed(2)}`)
                total += currAmount
            }) 
        } catch (err) {
            throw(err)
        ...
        const toAdd = {id: storage.get("purchases").value().length + 1, 
                       name: user.name, 
                       email: user.email, 
                       order: shoppingCart, 
                       total: total,
